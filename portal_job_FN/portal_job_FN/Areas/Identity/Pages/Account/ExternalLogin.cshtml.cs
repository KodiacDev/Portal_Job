            ...
            if (ModelState.IsValid) {
                ...
                if ((userWithexternalMail != null) && (Input.Email == externalMail))
                {
                    ...
                    if (resultAdd.Succeeded)
                    {
                        ...
                        // Gửi thông báo qua TempData hoặc query string
                        TempData["SuccessMessage"] = $"Liên kết tài khoản {userWithexternalMail.UserName} với {info.ProviderDisplayName} thành công";

                        return RedirectToPage("/Index", new { returnUrl = returnUrl });
                    }
                    else
                    {
                        TempData["ErrorMessage"] = "Liên kết thất bại";
                ...
                if (result.Succeeded) {
                    ...
                    if (result.Succeeded) {
                        ...
                        {
                            // Kích hoạt email nếu chưa xác nhận
                            ...
                            TempData["StatusMessage"] = "Đã tạo và liên kết tài khoản, kích hoạt email thành công.";

                            // Ví dụ: trở về trang chủ hoặc trang returnUrl
                            if (!string.IsNullOrEmpty(returnUrl))
                                return LocalRedirect(returnUrl);
                            else
                                return RedirectToPage("/Index"); // hoặc RedirectToAction("Index", "Home")
                        }
